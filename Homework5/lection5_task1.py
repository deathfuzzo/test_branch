...
from collections import Counter  # импортируем функцию подсчёта уникальных элементов последовательностей


def letter_stat(our_str):
    lst_data = list(our_str)  # создаем посимвольный список из строки
    lst_unique = []  # пустой список куда будут записываться количество число повторений для каждого из символов

    for i in lst_data:  #  циклом пробегаем символы строки и считаем количество вхождений символов в строку
        counter = lst_data.count(i)
        lst_unique.append(counter)  # добавляем количество с вхождений в пустой список

    letter_stat = dict(zip(lst_data, lst_unique))  # сращиваем 2 списка в словарь
    return letter_stat
