...
def scrabble(word):
    # задаем словарь по типу: буква = кол-во очков
    letters_points_dict = {"а": 1, "в": 1, "е": 1, "ё": 1, "и": 1, "н": 1, "о": 1, "р": 1, "с": 1, "т": 1, "д": 2,
                           "к": 2, "л": 2, "м": 2, "п": 2, "у": 2, "б": 3, "г": 3, "ь": 3, "я": 3, "й": 4, "ы": 4,
                           "ж": 5, "з": 5, "х": 5, "ц": 5, "ч": 5, "ф": 8, "ш": 8, "э": 8, "ю": 8, "щ": 10, "ъ": 15}
    word_lst = list(word)  # разбиваем входящее слово на буквы в список
    points = 0  #  задаём начальное значение очков
    for key in word_lst: #  пробегаем циклом список букв входящего слова
        key = letters_points_dict.get(key) #  возвращаем значение ключа (очки), если ключ (буква) найдена в словаре
        if key:
            points += key # суммируем значения ключей (очков) входящих в словарь
    return points
