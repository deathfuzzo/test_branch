...
def repeats(our_str):
    list_str = []  # создаём список, куда будем добавлять каждую букву и считать её вхождения в список всех букв
    new_str = ""
    for letter in our_str:  # пробегаем циклом все буквы
        if letter not in list_str:  # если буквы нет в списке, то записываем в счётчик 1 и добавляем букву в список
            new_str += str(letter) + "_" + str(1)
            list_str.append(letter)
        else:  # если буква уже есть в списке, то считаем кол-во её вхождений, прибавляем + 1 к счётчику и добавляем её в список
            counter = list_str.count(letter) + 1
            new_str += str(letter) + '_' + str(counter)
            list_str.append(letter)
    return new_str
